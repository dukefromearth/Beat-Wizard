...
void holdNoteOn() {
  ...
  if (tilt.getGyroY() > 450) {
    //get note value
    //currNote = getPitch(tilt.getAngleZ());

    //get volume value
    currVelocity = 90; //getVolume(tilt.getGyroX());

    //get effect number
    selectedEffect = calculateEffect();

    //    //get effect value
    //    currEffect = getEffect();
    //
    //
...
int getBeat() {
  ...
  btnNoteOnState = digitalRead(btnNoteOn);
  if (btnNoteOnState == 0) {
    if ((int)tilt.getAngleX() < -45) return kick;
    else if ((int)tilt.getAngleX() < 45) return snare;
    else if ((int)tilt.getAngleX() > 45) return crash;
  } else {
    if ((int)tilt.getAngleX() < -45) return hh;
    else if ((int)tilt.getAngleX() < 45) return tom1;
    else if ((int)tilt.getAngleX() > 45) return tom2;
  }
}
int getPitch(int x) {
